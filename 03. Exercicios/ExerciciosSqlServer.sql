...
SELECT TOP 10 ROW_NUMBER() OVER (ORDER BY(SUM(B.VAL_TOTAL) DESC) POSICAO,
	...
	D.NOME_CATEGORIA
	SUM(B.VALOR_TOTAL) AS TOTAL,
	100/@TOT_G*SUM(B.VALOR_TOTAL) AS PCT_PARTICIP
FROM VENDAS A WITH(NOLOCK)
	INNER JOIN VENDAS_ITENS B WITH(NOLOCK)
	ON A.NUM_VENDA = B.NUM_VENDA
	INNER JOIN PRODUTOS C WITH(NOLOCK)
...
SELECT MONTH(A.DATA_VENDA) AS MES,
 ...
 --ALIMENTANDO INFORMAÇOES NA SEGUNDA ETAPA
...
FROM (SELECT ID_CLIENTE,MAX(DATA_VENDA) AS DATA_VENDA FROM VENDAS WHERE STATUS 'N' GROUP BY ID_CLIENTE) AS X WITH(NOLOCK)
...
GROUP BY A.DATA_VENDA, X.DATA_VENDA 
...
FROM VENDAS B WITH(NOLOCK)
INNER JOIN VENDAS_ITENS C WITH(NOLOCK)
